/**
 * JustGage - a handy JavaScript plugin for generating and animating nice &
 * clean dashboard gauges. Copyright (c) 2012 Bojan Djuricic -
 * pindjur(at)gmail(dot)com | http://www.madcog.com Licensed under MIT. Date:
 * 31/07/2012
 * 
...
JustGage = function(x) {
	...
	this.config = {
		...
		showInnerShadow : (x.showInnerShadow != null) ? x.showInnerShadow
				: true,
		shadowOpacity : (x.shadowOpacity) ? x.shadowOpacity : 0.2,
		shadowSize : (x.shadowSize) ? x.shadowSize : 5,
		shadowVerticalOffset : (x.shadowVerticalOffset) ? x.shadowVerticalOffset
				: 3,
		levelColors : (x.levelColors) ? x.levelColors : percentColors,
		levelColorsGradient : (x.levelColorsGradient != null) ? x.levelColorsGradient
				: true,
		labelFontColor : (x.labelFontColor) ? x.labelFontColor : "#b3b3b3",
		startAnimationTime : (x.startAnimationTime) ? x.startAnimationTime
				: 700,
		startAnimationType : (x.startAnimationType) ? x.startAnimationType
				: ">",
		refreshAnimationTime : (x.refreshAnimationTime) ? x.refreshAnimationTime
				: 700,
	...
	var d = getStyle(document.getElementById(this.config.id), "width").slice(0,
			-2) * 1;
	var q = getStyle(document.getElementById(this.config.id), "height").slice(
			0, -2) * 1;
	var c, l;
	if ((d / q) > 1.25) {
		c = 1.25 * q;
	...
	var w = i + (c / 10)
			+ (c / 6.666666666666667 * this.config.gaugeWidthScale) / 2;
	...
	this.canvas.customAttributes.pki = function(K, L, N, E, O, F, D, y) {
		var B = (1 - (K - L) / (N - L)) * Math.PI, G = E / 2 - E / 10, J = G
				- E / 6.666666666666667 * y, C = E / 2 + F, A = O / 1.25 + D, H = E
				/ 2 + F + G * Math.cos(B), P = O - (O - A) + D - G
				* Math.sin(B), M = E / 2 + F + J * Math.cos(B), z = O - (O - A)
				+ D - J * Math.sin(B), I;
		I += "M" + (C - J) + "," + A + " ";
		I += "L" + (C - G) + "," + A + " ";
		I += "A" + G + "," + G + " 0 0,1 " + H + "," + P + " ";
	...
	this.txtTitle = this.canvas.text(this.params.titleX, this.params.titleY,
			this.config.title);
	this.txtTitle.attr({
	...
	this.txtValue = this.canvas.text(this.params.valueX, this.params.valueY,
			this.originalValue);
	this.txtValue.attr({
	...
	this.txtLabel = this.canvas.text(this.params.labelX, this.params.labelY,
			this.config.label);
	this.txtLabel.attr({
	...
	this.level.animate({
		pki : [ this.config.value, this.config.min, this.config.max,
				this.params.widgetW, this.params.widgetH, this.params.dx,
				this.params.dy, this.config.gaugeWidthScale ]
...
JustGage.prototype.refresh = function(b) {
	...
	var a = getColorForPercentage((b - this.config.min)
			/ (this.config.max - this.config.min), this.config.levelColors,
			this.config.levelColorsGradient);
	...
	this.canvas.getById(this.config.id + "-level").animate(
			{
				pki : [ b, this.config.min, this.config.max,
						this.params.widgetW, this.params.widgetH,
						this.params.dx, this.params.dy,
						this.config.gaugeWidthScale ],
				fill : a
...
JustGage.prototype.generateShadow = function(f, e) {
	...
	if (this.config.showInnerShadow == true) {
		this.canvas.canvas.childNodes[2].setAttribute("filter", "url(#"
				+ this.config.id + "-inner-shadow)");
		this.canvas.canvas.childNodes[3].setAttribute("filter", "url(#"
				+ this.config.id + "-inner-shadow)")
	}
};
	...
	for (var n = 0; n < g.length; n++) {
		if (b <= g[n].pct) {
			if (d == true) {
				...
				return "rgb("
						+ [ g[n].color.r, g[n].color.g, g[n].color.b ]
								.join(",") + ")"
			}
		}
	}
